...
def resolve_directory(argument: Optional[str]) -> str:
    ...
    while True:
        try:
            source = pending if pending is not None else read_user_input(_("Enter directory path to compress: "))
        ...
        if path:
            return path
        print(Fore.RED + _("Directory path cannot be empty.") + Style.RESET_ALL)
        pending = None
