...
Size thresholds prevent inefficient operations; `should_compress_file` in `src/file_utils.py` ignores files smaller than `MIN_COMPRESSIBLE_SIZE` (defined in `src/config.py`) to not bother compressing poorly compressible files.
