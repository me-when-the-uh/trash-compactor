...
The `is_file_compressed` function in `src/file_utils.py` compares these two values. A file is considered compressed if its on-disk size is smaller than its logical size.
...
Global projections accumulate the total logical bytes of candidate directories into `entropy_projected_original_bytes` and the estimated post-compression size into `entropy_projected_size`, both fields of `CompressionStats` in `src/stats.py`. At the end of a dry run, `print_entropy_dry_run` outputs a summary of these projections using `_format_summary_size` (also in `src/stats.py`), providing an upper bound on potential disk space recovery.
...
Classification that decides whether a file is skipped now happens primarily inside the Rust walker (`fast_walk`), which returns a category together with an on-disk size hint obtained via `GetCompressedFileSizeW`. Python-side recording in `CompressionStats` (via bulk skip ledgers or `record_file_skip_counters`) still accounts for those decisions using the same categories. The `record_file_skip` path and the older `should_compress_file` helper remain for debug paths and fallback logic.
...
Size thresholds prevent inefficient operations; the minimum size test (`MIN_COMPRESSIBLE_SIZE`, 8 KiB) is applied inside the native walker so that tiny files never even cross into Python during a normal run. The same constant is still referenced from `src/file_utils.py` for any paths that bypass the fast walker.
