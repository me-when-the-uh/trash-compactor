...
Finally, if entropy analysis is enabled, the program samples directory content to estimate compressibility. Directories with high entropy, indicating low potential savings, are excluded from traversal. Skipped directories are recorded in `CompressionStats`.
